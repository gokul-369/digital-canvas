...
class Media {
  ...
  update(scroll: { current: number; last: number }, totalHeight: number) {
    const half = totalHeight / 2;

    // calculate wrapped position
    const rawY = this.y + scroll.current;
    this.plane.position.x = this.x;
    this.plane.position.y = wrap(rawY, -half, half);
  ...
  onResize({
    ...
    this.x = col * this.width - ((this.cols - 1) * this.width) / 2;
    const totalRows = Math.ceil(this.length / this.cols);

    this.y = -row * this.height + ((totalRows - 1) * this.height) / 2;
  }
}
...
class App {
  ...
  createMedias(
    ...
    this.mediasImages = galleryItems; // just the original items
    // this.mediasImages = galleryItems.concat(galleryItems);

    const COLS = 3; // 👈 grid width (change 3 → 4 or 5)
    ...
    this.medias = this.mediasImages.map((data, index) => {
      ...
      return new Media({
        ...
        font,
        cols: COLS,
        total: this.mediasImages.length,
      });
    });
  }
  ...
  update() {
    ...
    if (this.medias && this.medias.length > 0) {
      const media0 = this.medias[0];
      const cols = media0.cols;
      const totalRows = Math.ceil(this.medias.length / cols);
      const totalHeight = media0.height * totalRows;

      this.medias.forEach((media) => media.update(this.scroll, totalHeight));
    }
...
export default function CircularGallery({
  ...
  return (
    //me
    <div
      className="w-dvw h-[700px] items-center justify-center overflow-hidden cursor-grab active:cursor-grabbing flex flex-wrap"
      ref={containerRef}
    />
  );
}
